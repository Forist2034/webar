...
instance ToCbor Timestamp where
  toCbor ts =
    Encoding
...
instance FromCbor Timestamp where
  fromCbor =
    Decoder
      ( do
...
instance ToCbor TimePeriod where
  toCbor (TimePeriod t0 t1) =
    Encoding (Enc.encodeTag 1003 <> Enc.encodeListLen 2)
      <> toCbor t0
      <> toCbor t1

instance FromCbor TimePeriod where
  fromCbor =
    Decoder
      ( do
          Dec.decodeTagCanonical >>= \case
